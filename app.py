    ...
    with col_c:
        ...
        if submitted:
            ...
            if ok:
                ...
                _u = username.strip().lower()
                _found = _u in USERS
                st.caption(f"🔍 Debug: usuario=`{_u}` | encontrado={_found} | "
                           f"usuarios_disponibles={list(USERS.keys())}")
...
def page_activities() -> None:
    ...
    st.markdown('<div class="section-header">🔢 Actividades del Proceso (22)</div>',
                unsafe_allow_html=True)
